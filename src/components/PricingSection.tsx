          ...
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`relative bg-white rounded-2xl shadow-soft border-2 transition-all duration-300 hover:shadow-strong ${
                plan.popular ? 'border-orange-200 ring-2 ring-orange-100' : 
                plan.premium ? 'border-purple-200 ring-2 ring-purple-100' : 
            ...
            >
              ...
              <div className="p-8">
                ...
                <div className="text-center mb-8">
                  ...
                  <p className="text-primary-600 mb-4">{plan.subtitle}</p>
                  
                  {/* Platforms */}
                  <div className="flex flex-wrap justify-center gap-2 mb-6">
                    {plan.platforms.map((platform, idx) => (
                      <div key={idx} className="flex items-center space-x-1 text-xs text-primary-600 bg-primary-50 px-2 py-1 rounded-full">
                  ...
                  {/* Pricing */}
                  <div className="mb-4">
                    <div className="text-4xl font-bold text-primary-950 mb-2">{plan.price}</div>
                    <div className="text-primary-600 mb-2">{plan.paymentTerms}</div>
                    <div className="text-lg font-semibold text-primary-800">
                      {plan.monthlyPrice}
                      {plan.duration && <span className="text-sm font-normal text-primary-600">{plan.duration}</span>}
                    </div>
                  </div>
                ...
                <div className="space-y-3 mb-8">
                  {plan.features.map((feature, idx) => (
                    <div key={idx} className="flex items-start space-x-3">
                      ...
                      <span className="text-sm text-primary-700 leading-relaxed">{feature}</span>
                    </div>
                  ))}
                </div>
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  className={`w-full py-4 px-6 rounded-xl font-semibold text-sm transition-all duration-300 group ${
                    plan.popular 
                      ? 'bg-orange-500 text-white hover:bg-orange-600 shadow-lg hover:shadow-xl' 
                      : plan.premium
                      ? 'bg-purple-500 text-white hover:bg-purple-600 shadow-lg hover:shadow-xl'
                      : 'bg-primary-950 text-white hover:bg-primary-800 shadow-lg hover:shadow-xl'
                  }`}
                >
                  {plan.cta}
                ...
                <div className="mt-4 text-center space-y-1">
                  {plan.highlights.map((highlight, idx) => (
                    <div key={idx} className="text-xs text-primary-600">{highlight}</div>
                  ))}
                </div>
              </div>
            </motion.div>
